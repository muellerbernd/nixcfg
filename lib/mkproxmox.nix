...
let
  user_folder_names = nixpkgs.lib.attrNames (
    nixpkgs.lib.filterAttrs (n: v: v == "directory") (builtins.readDir (builtins.toString ../users))
  );
  home_manager_users =
    nixpkgs.lib.lists.intersectLists (nixpkgs.lib.lists.intersectLists users user_folder_names)
      [ "bernd" ];
  ...
  student_user_cfgs = nixpkgs.lib.forEach students (student: mkUser { name = student; });
in
lib.nixosSystem rec {
  inherit system;
